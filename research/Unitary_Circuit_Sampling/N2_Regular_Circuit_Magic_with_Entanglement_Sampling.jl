...
function N2_Regular_Circuit_Magic_w_Entanglement_sampling(No_Samples::Int, Seed::Int)
    ...
    PauliOperators = Dict();
    PauliOperators[1] = Measure_Magic.PauliOperatorList(Measure_Magic.GenerateAllPauliStrings(1), 1)
    PauliOperators[2] = Measure_Magic.PauliOperatorList(Measure_Magic.GenerateAllPauliStrings(2), 2)
    ...
    for i in ProgressBar(1:No_Samples)    
        ...
        for q in 1:(No_Qubits - 1)
            ...
            push!(SubMagic, Measure_Magic.MeasureMagic_Mixed(reduced_matrix_left,   PauliOperators[1], 2))
            push!(SubMagic, Measure_Magic.MeasureMagic_Mixed(reduced_matrix_right,  PauliOperators[1], 2))
            push!(Pure, sum(diag(reduced_matrix_left^2)))
            push!(Pure, sum(diag(reduced_matrix_right^2)))
        end

        push!(Entanglement, Entropies)
        push!(SubSystems, SubSys)
        push!(Magic, Measure_Magic.MeasureMagic_Pure(State, PauliOperators[2], 2))
...
M, S, SM, SS, Pu = N2_Regular_Circuit_Magic_w_Entanglement_sampling(2^18, 2)
SML = map(first, SM)
SMR = map(last, SM)
S   = map(first, S)
Pu  = map(first, Pu)