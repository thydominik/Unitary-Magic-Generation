module Measure_Entropy
using LinearAlgebra
export calculate_Renyi_entropy
    ...
    function Renyi_entropy(ρ::Matrix{Complex{Float64}}, α::Float64)
        ...
        if α == 1
            return von_neumann_entropy(ρ)
        else
            entropy = (1 / (1 - α)) * log2(sum(abs.(eigenvalues) .^ α))
        end

        return entropy
    end

    function calculate_Renyi_entropy(state_vector::Vector{Complex{Float64}}, α; subsystems::String = "All")
